...
bool VART::XmlScene::LoadFromFile(const std::string& fileName)
{
    ...
    { //XML parser sucessfull
        ...
        LoadScene( VART::File::GetPathFromString(fileName) );
        // Terminate Xerces
        Terminate();
...
void VART::XmlScene::LoadScene(const std::string& basePath)
{
    ...
    for (i = 0; i < sceneList->getLength(); i++)
    {
        ...
        for(j = 0; j<nodesL->getLength(); j++)
        {
            ...
            else if (TempCString(nodesL->item(j)->getNodeName()) == "node")
            {
                ...
                // Is it a light?
                lightPtr = dynamic_cast<VART::Light*>(nodePtr);
                if (lightPtr)
                    AddLight(lightPtr);
                else
                {
