...
VART::GraphicObj* VART::Scene::GetObject(unsigned int pickName)
{
    ...
    {
        (*iter)->TraverseDepthFirst(&finder);
        ++iter;
    }
...
bool VART::Scene::DrawLightsOGL() const {
    ...
    {
		glPushMatrix();
		glMultMatrixd((*iter)->GetTransform()->GetData());
...
void VART::Scene::UseNextCamera() {
    ++currentCamera;
    if (currentCamera == cameras.end())
        currentCamera = cameras.begin();
}

void VART::Scene::UsePreviousCamera() {
    --currentCamera;
    if (currentCamera == cameras.end())
        --currentCamera;
}
...
{
#ifdef VART_OGL
    const int SELECTION_BUFFER_SIZE = 512; // FixMe: find a better place to put this!
    GLint viewport[4]; // viewport coordinates
    GLint hits; // number of primites picked
    ...
    while (hits > 0) // for each hit record...
    {
        ...
        while (numNames > 0) // for each name in hit record...
        {
            ...
            pickPtr = GetObject(selectBufPtr[nameIndex]);
...
#else
    cerr << "Error: Scene::Pick implemented ONLY using OpenGL!" << endl;
#endif
}
